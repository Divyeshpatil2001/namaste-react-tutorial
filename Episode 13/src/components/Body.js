...
const Body = () => {
  ...
  // promoted hoc
  const RestaurantCarPromoted = WithPromotedLabel(RestaurantCard);

  useEffect(() => {
    console.log("useffect called");
    fetchData();
  }, []); //first argument is call back function and second argument is dependency array
  console.log("use 11called", listofRestaurants);
  ...
  const fetchData = async () => {
    try {
      ...
      if (restaurants) {
        const updatedRestaurants = restaurants.map((res, index) => {
          return {
            ...res,
            info: {
              ...res.info,
              isPromoted: index < 5, // make true for first 5
            },
          };
        });
        setListOfRestaurants(updatedRestaurants);
        setFilterRestaurants(updatedRestaurants);
      } else {
  ...
  if (onlineStatus === false)
    return <h1>Looks like you're offline !!! pleasse check your connection</h1>;
    ...
    <div className="body">
      ...
      <div className="flex flex-wrap">
        {filterRestaurants.map((restaurant) => (
          ...
          >
            {/* for promoted using hoc */}
            {restaurant.info.isPromoted ? <RestaurantCarPromoted resData={restaurant}/> : <RestaurantCard resData={restaurant}/>}
